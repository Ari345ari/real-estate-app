...
router.post('/card', authenticateToken, async (req, res) => {
  const { card_number, expiry, billing_address_id } = req.body;
  try {
    const card = await pool.query(
      `INSERT INTO "Card" (User_ID, Card_Number, Expiry, Billing_Address_ID) VALUES ($1, $2, $3, $4) RETURNING *`,
      [req.user.userId, card_number, expiry, billing_address_id]
    );
